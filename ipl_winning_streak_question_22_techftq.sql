...
--select * from ipl_results where home_team = 'Sunrisers Hyderabad' or away_team ='Sunrisers Hyderabad'
...
(
select * ,  row_number() over(partition by teams order by teams,dates) as id from cte c
inner join ipl_results i
on c.teams = i.home_team or c.teams = i.away_team
) , cte3 as 
(
select * , id - row_number() over(partition by teams order by teams,dates) as winning_streak   from cte2 where result = teams
)
,cte4 as (
select teams , count(winning_streak) as winning_streak  from cte3
group by teams,winning_streak
)

select teams , MAX(winning_streak) as winning_streak from cte4
group by teams


--select teams , MAX(winning_streak) as winning_streak from cte3
--group by teams
