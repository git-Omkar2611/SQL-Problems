...
with cte as (
select pv.* ,  datediff(SECOND,session_starttime,session_endtime) as diff_seconds from user_sessions us
inner join post_views pv
on us.session_id = pv.session_id
)

select post_id , sum((perc_viewed/100)*diff_seconds) as total_time from cte 
group by post_id
HAVING sum((perc_viewed/100)*diff_seconds) > 5